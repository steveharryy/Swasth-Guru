...
export default function PatientDashboard() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const upcoming = userAppointments.filter((apt: Appointment) => {
            ...
            return (status === 'upcoming' || status === 'confirmed' || status === 'pending') && normalizedAptDate >= todayStr;
          });
    ...
    // Update medical records count in stats
    setStatsData(prev => ({
      ...prev,
      medicalRecords: totalRecords.toString(),
      healthScore: totalRecords > 0 ? "90%" : "100%"
    }));
