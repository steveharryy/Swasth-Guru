    ...
    switch (targetStatus) {
      case 'upcoming':
        filtered = filtered.filter(apt => {
          ...
          return normalizeDate(apt.date) >= todayStr && isStatusUpcoming;
        });
        break;
      case 'completed':
        filtered = filtered.filter(apt => {
...
function AppointmentCard({
  ...
  const isToday = normalizeDate(appointment.date) === normalizeDate(new Date().toISOString().split('T')[0]);
              ...
              {isUpcoming && isToday && (
                <>
                  {timeStatus === 'ready' && (
                    <Button
                      variant="premium"
                      className="flex-1 h-11 text-base font-bold rounded-xl border-2 border-primary/20"
